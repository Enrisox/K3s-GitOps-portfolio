...
In questo step vediamo come Kubernetes può scalare automaticamente il numero di repliche di un Deployment in base al 
...
## Creazione di HPA per mio progetto
...
kubectl autoscale deployment portfolio \
  ...
  --max=5
```
**Cambio il file HTML dentro a configmap.yaml e modifco deployment.yaml sempre dalla repository , in modo che parta un refresh automatico su ArgoCD e il mio portfolio web si aggiorni.**
